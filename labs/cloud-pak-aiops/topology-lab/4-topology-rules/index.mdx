...
This is where the topology data coming from the **service** topology source
is mapped onto the topology data coming from the **topology** source. 
In our hypothetical scenario, we know we have Resources of type
"device" in both sets of topology, and we know that they relate to the same
...
In our case, we have a Resource called "asua01" and "buea01" represented in our
infrastructure topology data as well as in our service data. Our goal is to
...
any other devices included in our two data sets, they too would be merged
...
- Expand the **Conditions** drop-down.
  ...
  - **Providers**: Add both of your jobs to the list (e.g.
    **FILE.OBSERVER:topology.txt** and **FILE.OBSERVER:service.txt**)
  ...
  - **Resource types**: Add the Resource type that this rule should apply to
    which is **device**. If this section was left empty, it would try to apply
...
time search for **asua01**. Now you will see just one instance in the list
of Resources called asua01 and one called buea01 whereas before you would have seen two.
You will also how many resources were merged see on the bottom.

![](images/computer.png)

The Resource asua01 and buea01 is only shown once in the topology, but remains
